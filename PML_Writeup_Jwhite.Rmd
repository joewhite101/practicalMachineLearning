...
The section on Parameter Exploration below examines engineering the parameters to select a different parameter set. The model that was used included all of the data as predictors. In order to use all of the predictors the data is parallelized across 12 CPU cores prior to training.
...
newModel <- train(classe ~.,data=updated, method="rf", ntree=200 )
```

```{r echo=FALSE}
newModel <- readRDS("newMod.rds")
```

The updated model has an accuracy rate 99.91% of and an OOB estimated error of 0.05% which are both superior to the model with all variables included:
```{r}
newModel
newModel$finalModel
```
...
Finally a prediction is generated with the updated model using the smaller parameter set:
